...
esp_err_t cors_options_handler(PsychicRequest *request) {
    return request->reply_cors(200, "text/plain", "allowed");
};
...
esp_err_t PsychicHttpServer::_start()
{
  esp_err_t ret;
    ...
    Serial.println("started async workers");
  #endif
...
esp_err_t PsychicHttpServer::_startServer() {
  Serial.println("http _startServer");
...
{
  //inject correct cors allow origin
  if (method != HTTP_OPTIONS) {
    this->on(uri, HTTP_OPTIONS, cors_options_handler);
  }
