...
esp_err_t PsychicRequest::reply_cors(int code, const char *contentType, const char *content)
{
  PsychicResponse response(this);
  ...
  response.setContentType(contentType);
  response.setContent(content);
  response.addHeader("Access-Control-Allow-Origin", this->header("Origin").c_str());
